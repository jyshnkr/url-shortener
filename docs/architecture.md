...
- **Built:** startup, health check, versioned table setup and link creation with destination reuse.
- **Later:** redirects, expiration, analytics and optional access protection are not implemented.
...
flowchart LR
    ...
    H -->|"201 created / 200 reused, or safe error"| C
```
...
| --- | --- |
| `controller` | HTTP input, status selection and safe error translation |
| `model` | Separate request, response, saved link and creation outcome records |
| `service` | Validation, lookup, creation and bounded retry coordination |
| `dao` | SQL, row mapping, fingerprint lookup/full comparison and storage-failure translation |
| `validation` | Destination URL rules |
| `generator` | Secure random short codes; a supplier seam permits forced test collisions |
| `exception` | Application failures without HTTP or SQL details |
...
- `GET /r/{code}`: look up the mapping; return 302, destination `Location`, `Cache-Control: no-store`, or 404 for an unknown code. The browser opens the destination; the service does not fetch it.
