...
class CreationLifecycleIT {
  ...
  void aSilentDatabaseNetworkStallReturns503WithinTheClientDeadline() throws Exception {
    try (var database = new PostgreSQLContainer("postgres:17.11")) {
      ...
      try (var proxy = new StallingDatabaseProxy(database.getHost(), database.getMappedPort(5432));
          var application = start(database, "https://short.example",
              "jdbc:postgresql://127.0.0.1:" + proxy.port() + "/" + database.getDatabaseName())) {
        assertThat(create(application, UUID.randomUUID().toString()).statusCode()).isEqualTo(201);
        proxy.stall();
        try {
  ...
  void anActualDatabaseOutageReturns503WithoutExposingInternalDetails() throws Exception {
    try (var database = new PostgreSQLContainer("postgres:17.11")) {
      ...
      try (var application = start(database, "https://short.example")) {
        ...
        assertThat(error.get("detail").stringValue()).contains("Retry", "destinationUrl");
  ...
  @Test
  void aFingerprintCandidateWithDifferentTextReturnsSafe503InsteadOfTheWrongLink() throws Exception {
    try (var database = new PostgreSQLContainer("postgres:17.11")) {
      database.start();
      try (var application = start(database, "https://short.example")) {
        ...
        jdbc.execute("ALTER TABLE short_links DROP CONSTRAINT short_links_destination_hash_matches");
        jdbc.update("UPDATE short_links SET destination_hash = sha256(convert_to(?, 'UTF8'))",
        ...
        assertThat(error.get("status").intValue()).isEqualTo(503);
        assertThat(error.get("detail").stringValue()).contains("Retry", "destinationUrl");
        assertThat(response.body()).doesNotContain("original", "short.example", "SELECT", "Exception");
  ...
  private ConfigurableApplicationContext start(
    ...
    return new SpringApplicationBuilder(UrlShortenerApplication.class).run(
        "--server.port=0",
  ...
  private HttpResponse<String> create(ConfigurableApplicationContext application, String identity)
    ...
    var request = HttpRequest.newBuilder(URI.create("http://127.0.0.1:" + port + "/api/v1/links"))
        ...
        .POST(HttpRequest.BodyPublishers.ofString("{\"destinationUrl\":\"https://example.com/persisted/" + identity + "\"}"))
        .build();
